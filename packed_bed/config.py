...
class ModelConfig(FrozenConfigModel):
    ...
    axial_cells: int
    axial_face_positions_m: tuple[float, ...] | None = None
    ...
    @field_validator("axial_face_positions_m", mode="before")
    @classmethod
    def coerce_axial_face_positions(cls, value: Any) -> tuple[Any, ...] | None:
        if value is None:
            return None
        return _as_tuple(value)

    @field_validator("axial_face_positions_m")
    @classmethod
    def validate_axial_face_positions(cls, value: tuple[float, ...] | None) -> tuple[float, ...] | None:
        if value is None:
            return None
        if len(value) < 2:
            raise ValueError("must contain at least two face positions.")
        if not math.isclose(float(value[0]), 0.0, rel_tol=0.0, abs_tol=1.0e-12):
            raise ValueError("must start at 0.0 m.")
        if any(float(position_b) <= float(position_a) for position_a, position_b in zip(value, value[1:], strict=True)):
            raise ValueError("must be strictly increasing.")
        return tuple(float(position) for position in value)

    @model_validator(mode="after")
    def validate_axial_grid(self) -> "ModelConfig":
        if self.axial_face_positions_m is None:
            return self
        if len(self.axial_face_positions_m) != self.axial_cells + 1:
            raise ValueError("axial_face_positions_m must contain axial_cells + 1 entries.")
        if not math.isclose(
            float(self.axial_face_positions_m[-1]),
            float(self.bed_length_m),
            rel_tol=0.0,
            abs_tol=1.0e-12,
        ):
            raise ValueError("axial_face_positions_m must end at bed_length_m.")
        return self
