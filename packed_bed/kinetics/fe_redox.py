...
def _equilibrium_constant_value(a: float, b: float, temperature_k: float) -> float:
    return math.exp(a + b / temperature_k)


def equilibrium_constant_h2_fe3o4_to_feo_value(temperature_k: float) -> float:
    return _equilibrium_constant_value(*H2_EQUILIBRIUM_COEFFICIENTS["Fe3O4_to_FeO"], temperature_k)


def equilibrium_constant_h2_feo_to_fe_value(temperature_k: float) -> float:
    return _equilibrium_constant_value(*H2_EQUILIBRIUM_COEFFICIENTS["FeO_to_Fe"], temperature_k)


def equilibrium_constant_co_fe3o4_to_feo_value(temperature_k: float) -> float:
    return _equilibrium_constant_value(*CO_EQUILIBRIUM_COEFFICIENTS["Fe3O4_to_FeO"], temperature_k)


def equilibrium_constant_co_feo_to_fe_value(temperature_k: float) -> float:
    return _equilibrium_constant_value(*CO_EQUILIBRIUM_COEFFICIENTS["FeO_to_Fe"], temperature_k)


def _arrhenius_value(preexponential: float, activation_energy_j_per_mol: float, temperature_k: float) -> float:
    return preexponential * math.exp(-activation_energy_j_per_mol / (GAS_CONSTANT_J_PER_MOL_K * temperature_k))


def h2_reaction_constant_value(rate_key: str, *, temperature_k: float) -> float:
    return _arrhenius_value(
        H2_REDUCTION_PREEXPONENTIALS[rate_key],
        H2_REDUCTION_ACTIVATION_ENERGIES_J_PER_MOL[rate_key],
        temperature_k,
    )


def h2_diffusivity_value(rate_key: str, *, temperature_k: float) -> float:
    return _arrhenius_value(
        H2_DIFFUSIVITY_PREEXPONENTIALS[rate_key],
        H2_DIFFUSIVITY_ACTIVATION_ENERGIES_J_PER_MOL[rate_key],
        temperature_k,
    )


def co_reaction_constant_value(rate_key: str, *, temperature_k: float) -> float:
    return _arrhenius_value(
        CO_REDUCTION_PREEXPONENTIALS[rate_key],
        CO_REDUCTION_ACTIVATION_ENERGIES_J_PER_MOL[rate_key],
        temperature_k,
    )


def feo_co_diffusivity_value(*, temperature_k: float) -> float:
    return _arrhenius_value(
...
def ch4_reaction_constant_value(*, temperature_k: float) -> float:
    return _arrhenius_value(
...
def oxidation_o2_rate_constant_value(*, temperature_k: float) -> float:
    return _arrhenius_value(
...
def FEO_CO_STRUCTURAL_PARAMETER(s0_per_m: float, l0_per_m2: float, epsilon0: float) -> float:
    return 4.0 * math.pi * l0_per_m2 * (1.0 - epsilon0) / (s0_per_m ** 2)


def fe2o3_h2_reduction_rate_value(
    *,
    temperature_k: float,
    c_fe2o3_mol_per_m3: float,
    c_h2_mol_per_m3: float,
    c_h2o_mol_per_m3: float,
    x_fe2o3: float,
) -> float:
...
def fe3o4_h2_reduction_rate_value(
    *,
    temperature_k: float,
    c_fe3o4_mol_per_m3: float,
    c_h2_mol_per_m3: float,
    c_h2o_mol_per_m3: float,
    x_fe3o4: float,
) -> float:
...
def feo_h2_reduction_rate_value(
    *,
    temperature_k: float,
...
def fe2o3_co_reduction_rate_value(
    *,
    temperature_k: float,
    oc_mass_density_kg_per_m3: float,
...
def fe3o4_co_reduction_rate_value(
    *,
    temperature_k: float,
    oc_mass_density_kg_per_m3: float,
    c_co_mol_per_m3: float,
    ...
    return (
        oc_mass_density_kg_per_m3
        * co_reaction_constant_value("Fe3O4", temperature_k=temperature_k)
        * max(driving_force, 0.0)
        * (1.0 - min(max(x_fe3o4, 0.0), 1.0)) ** 1.2
    )


def feo_co_reduction_rate_value(
    *,
    temperature_k: float,
    c_feo_mol_per_m3: float,
    c_co_mol_per_m3: float,
    ...
    x_safe = min(max(x_feo, 0.0), 1.0 - 1.0e-12)
    psi = FEO_CO_STRUCTURAL_PARAMETER(FEO_CO_S0_PER_M, FEO_CO_L0_PER_M2, FEO_CO_EPSILON0)
    structure_term = math.sqrt(max(1.0 - psi * math.log(1.0 - x_safe), 1.0e-16))
    beta = (
        2.0
        * co_reaction_constant_value("FeO", temperature_k=temperature_k)
        * (1.0 - FEO_CO_EPSILON0)
        / (FEO_MOLAR_VOLUME_M3_PER_MOL * FEO_CO_S0_PER_M * feo_co_diffusivity_value(temperature_k=temperature_k))
    )
    denominator = 1.0 + beta * FEO_CO_ALPHA / psi * (structure_term - 1.0)
    driving_force = c_co_mol_per_m3 - c_co2_mol_per_m3 / equilibrium_constant_co_feo_to_fe_value(temperature_k)
    return (
        FEO_CO_S0_PER_M
        * c_feo_mol_per_m3
        / (1.0 - FEO_CO_EPSILON0)
...
def fe2o3_ch4_reduction_rate_value(
    *,
    temperature_k: float,
    oc_mass_density_kg_per_m3: float,
    c_ch4_mol_per_m3: float,
    x_fe2o3: float,
) -> float:
    return (
        oc_mass_density_kg_per_m3
        * ch4_reaction_constant_value(temperature_k=temperature_k)
        * max(c_ch4_mol_per_m3, 0.0)
        * (1.0 - min(max(x_fe2o3, 0.0), 1.0)) ** (2.0 / 3.0)
    )


def fe_o2_oxidation_rate_fe_value(
    *,
    temperature_k: float,
    oc_mass_density_kg_per_m3: float,
    p_o2_pa: float,
    ...
    r_o2 = (
        oc_mass_density_kg_per_m3
        * oxidation_o2_rate_constant_value(temperature_k=temperature_k)
        * (1.0 - min(max(x_fe, 0.0), 1.0))
        * p_o2_bar
    )
    return r_o2 / 0.75  # convert from O2 basis to Fe basis
...
def he_feo_co_reduction(context: KineticsContext):
    ...
    psi_value = FEO_CO_STRUCTURAL_PARAMETER(FEO_CO_S0_PER_M, FEO_CO_L0_PER_M2, FEO_CO_EPSILON0)
    psi = Constant(psi_value)
...
__all__ = [
    ...
    "pressure_bar_value",
    "equilibrium_constant_h2_fe3o4_to_feo_value",
    "equilibrium_constant_h2_feo_to_fe_value",
    "equilibrium_constant_co_fe3o4_to_feo_value",
    "equilibrium_constant_co_feo_to_fe_value",
    "h2_reaction_constant_value",
    "h2_diffusivity_value",
    "co_reaction_constant_value",
    "feo_co_diffusivity_value",
    "ch4_reaction_constant_value",
    "oxidation_o2_rate_constant_value",
