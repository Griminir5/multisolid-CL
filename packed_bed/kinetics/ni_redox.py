...
GAS_CONCENTRATION_GATE_MOL_PER_M3 = 1.0e-8
SOLID_REACTANT_FRACTION_GATE = 1.0e-8
MIN_PRESSURE_BAR = 1.0e-12


NI_REDOX_RATE_COEFFICIENTS = {
    "h2_reduction": 9.00e-4,
    "co_reduction": 3.50e-3,
    "ch4_reduction": 1.20e-2,
...
# Hamers pressure correction used with the Medrano shrinking-core constants.
# Eq. 13 uses gas-phase C_g; solver c_gas is stored per total bed volume.
NI_REDOX_PRESSURE_EXPONENTS = {
    "h2_reduction": 0.75,
    "co_reduction": 0.85,
    "ch4_reduction": 0.95,
    "o2_oxidation": 1.05,
...
def gas_phase_concentration_value(
    *,
    total_pressure_pa: float,
    mole_fraction: float,
    temperature_k: float,
) -> float:
    return max(total_pressure_pa, 0.0) * max(mole_fraction, 0.0) / (
...
def active_ni_concentration_value(
    ...
    return max(ni_concentration_mol_per_m3, 0.0) + max(nio_concentration_mol_per_m3, 0.0)
...
def rate_constant_value(
    ...
    *,
    temperature_k: float,
    total_pressure_pa: float = PRESSURE_PA_PER_BAR,
) -> float:
    coefficient = NI_REDOX_RATE_COEFFICIENTS[rate_key]
    activation_energy = NI_REDOX_ACTIVATION_ENERGIES_J_PER_MOL[rate_key]
    pressure_exponent = NI_REDOX_PRESSURE_EXPONENTS[rate_key]
    pressure_bar = max(pressure_bar_value(total_pressure_pa), MIN_PRESSURE_BAR)
    return (
        coefficient
        * math.exp(-activation_energy / (GAS_CONSTANT_J_PER_MOL_K * temperature_k))
        * pressure_bar ** (-pressure_exponent)
    )
...
def conversion_rate_value(
    ...
    *,
    ...
    gas_concentration_available = max(gas_concentration_mol_per_m3, 0.0)
    gas_concentration_safe = gas_concentration_available + MIN_GAS_CONCENTRATION_MOL_PER_M3
    gas_gate = (
        gas_concentration_available / (gas_concentration_available + GAS_CONCENTRATION_GATE_MOL_PER_M3)
        if gas_concentration_available > 0.0
        else 0.0
    )
    solid_gate = one_minus_x / (one_minus_x + SOLID_REACTANT_FRACTION_GATE)
    return gas_gate * solid_gate * 3.0 * b * gas_concentration_safe**n / (r0 * c_s * denominator)
...
def ch4_reduction_rate_value(
    *,
    ...
    dxdt = conversion_rate_value(
        "ch4_reduction",
        temperature_k=temperature_k,
        gas_concentration_mol_per_m3=c_ch4_mol_per_m3,
        conversion=x_reduction,
        total_pressure_pa=total_pressure_pa,
    )
    return 0.25 * c_active_ni_mol_per_m3 * dxdt
...
def _pressure_bar_expression(pressure) -> Any:
    return Max(pressure / Constant(PRESSURE_PA_PER_BAR * Pa), Constant(MIN_PRESSURE_BAR))
...
def _rate_constant_expression(rate_key: str, temperature_k, pressure_bar) -> Any:
    coefficient = NI_REDOX_RATE_COEFFICIENTS[rate_key]
    activation_energy = NI_REDOX_ACTIVATION_ENERGIES_J_PER_MOL[rate_key]
    pressure_exponent = NI_REDOX_PRESSURE_EXPONENTS[rate_key]
    return (
        ...
        * pressure_bar ** Constant(-pressure_exponent)
    )
...
def _gas_phase_concentration_expression(context: KineticsContext, species_id: str, temperature_k):
    ...
    gas_mole_fraction = Max(context.model.y_gas(species_idx, context.idx_cell), Constant(0.0))
    return _pressure_pa_expression(context.model.P(context.idx_cell)) * gas_mole_fraction / (
...
def _solid_concentration_expression(context: KineticsContext, species_id: str):
    ...
    return Max(concentration, Constant(0.0))
...
def _ni_redox_terms(context: KineticsContext) -> NiRedoxTerms:
    ...
    pressure_bar = _pressure_bar_expression(context.model.P(context.idx_cell))
    ...
    return NiRedoxTerms(
        ...
        c_ch4_mol_per_m3=_optional_gas_phase_concentration_expression(context, "CH4", temperature_k),
...
def _conversion_rate_expression(rate_key: str, temperature_k, pressure_bar, gas_concentration_mol_per_m3, conversion) -> Any:
    c_s = NI_REDOX_SOLID_CONCENTRATIONS_MOL_PER_M3[rate_key]
    r0 = NI_REDOX_GRAIN_RADII_M[rate_key]
    n = NI_REDOX_REACTION_ORDERS[rate_key]
    b = NI_REDOX_B[rate_key]

    conversion_bounded = Min(Constant(1.0), Max(conversion, Constant(0.0)))
    one_minus_x = Max(Constant(1.0) - conversion_bounded, Constant(MIN_ONE_MINUS_CONVERSION))
    gas_concentration_available = Max(gas_concentration_mol_per_m3, Constant(0.0))
    gas_concentration_safe = gas_concentration_available + Constant(MIN_GAS_CONCENTRATION_MOL_PER_M3)
    gas_gate = gas_concentration_available / (
        gas_concentration_available + Constant(GAS_CONCENTRATION_GATE_MOL_PER_M3)
    )
    solid_gate = one_minus_x / (one_minus_x + Constant(SOLID_REACTANT_FRACTION_GATE))

    denominator = (
        Constant(1.0)/ _rate_constant_expression(rate_key, temperature_k, pressure_bar) * one_minus_x ** Constant(-2.0 / 3.0)
        + Constant(r0)/ _diffusivity_expression(rate_key, temperature_k, conversion_bounded) * (one_minus_x ** Constant(-1.0 / 3.0) - Constant(1.0))
    )

    return (
        gas_gate
        * solid_gate
        * Constant(3.0 * b / (r0 * c_s))
        * gas_concentration_safe ** Constant(n)
        / denominator
    )
...
@register_kinetics_hook("ni_redox_reduction_ch4")
def ni_redox_reduction_ch4(context: KineticsContext):
    terms = _ni_redox_terms(context)
    dxdt_expression = _conversion_rate_expression(
        "ch4_reduction",
        terms.temperature_k,
        terms.pressure_bar,
        terms.c_ch4_mol_per_m3,
        terms.x_reduction,
    )
    # One mole of CH4 removes the lattice oxygen of four moles of NiO.
    return Constant(0.25 * mol / (m**3 * s)) * terms.c_active_ni_mol_per_m3 * dxdt_expression
