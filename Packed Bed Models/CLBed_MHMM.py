...
# ["CaAl2O4", "Ni", "NiO", "Fe", "FeO", "Fe3O4", "Fe2O3", "Ca", "CaO", "CaCO3", "CaSO4"]
...
class CLBed_mass(daeModel):
    def __init__(
        ...
        self.c_in = daeParameter("conc_inlet", mol / (m**3), self, "Legacy inlet concentration parameter retained for compatibility")
    ...
    def DeclareEquations(self):
        ...
        eq = self.CreateEquation("Active_inlet_temperature")
        eq.Residual = self.T_in() - self.T_in_const()
        ...
        eq.Residual = (self.P(Nc - 1) - self.P_out_const()) / dx - ergun_drag
...
class simBed(daeSimulation):
    ...
    def SetUpParametersAndDomains(self):
        ...
        self.model.e_b.SetValues(0.5+np.zeros(self.model.x_centers.NumberOfPoints, dtype=float))
        self.model.e_p.SetValues(0.5+np.zeros(self.model.x_centers.NumberOfPoints, dtype=float))
