...
from pyUnits import m, s, K, mol, Pa, GW # this will not show up because pylance cannot get to .pyd files

#
molar_flux_type = daeVariableType(name="molar_flux_type", units=mol/(s*m**2),
                                  lowerBound=-100, upperBound=100000, initialGuess=0, absTolerance=1e-5)
molar_flow_type = daeVariableType(name="molar_flow_type", units=mol/s,
                                  lowerBound=-10, upperBound=10000, initialGuess=0, absTolerance=1e-5)
    ...
    def DeclareEquations(self):
        ...
        for face_index in range(1, Nf - 1):
            ...
            eq.Residual = (
                ...
                - uplus * self.c_gas(idx_gas, idx_cell_L) # Left-biased upwind approximation for face value of a variable that is defined in cells
                - uminus * self.c_gas(idx_gas, idx_cell_R) # Right-biased upwind approximation for face value of a variable that is defined in cells
                + self.Dax(face_index) * ct_face * (self.y_gas(idx_gas, idx_cell_R) - self.y_gas(idx_gas, idx_cell_L)) / dx # approximation of spatial derivative at face
            )
...
class simBed(daeSimulation):
    ...
    def SetUpParametersAndDomains(self):
        ...
        self.model.F_in_const.SetValue(.785 * mol/s)
