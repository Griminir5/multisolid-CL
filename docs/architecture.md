...
| Axial schemes | `reconstruct_forward_face_state` builds the one-sided forward-flow expression; `reconstruct_face_states` and `split_face_flux` provide optional two-sided interior transport. | Add the public id to `SUPPORTED_SCHEMES` and its boundary-safe reconstruction in [`axial_schemes.py`](../packed_bed/axial_schemes.py). `simulation.interior_flow_mode` selects the model path; boundaries currently support forward flow only. |
